...
class ProjectData:
    def __init__(self, csv_path: Path) -> None:
        ...
        self.df = pd.read_csv(self.project_path.joinpath(csv_path))
...
def get_project_data(
    ...
    return ProjectData(csv_path)


@lru_cache(typed=True)
def get_ml_data(
    project_data: ProjectData, test_size: float = 0.2, random_seed: int = 42
    ) -> MLData:
    return MLData(project_data, test_size, random_seed)
