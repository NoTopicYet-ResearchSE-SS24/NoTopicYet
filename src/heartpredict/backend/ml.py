...
def classification_for_different_classifiers(x_train, y_train, x_valid, y_valid):
    ...
    max_tree_depth = _calculate_max_tree_depth(x_train.shape[1])
    k_min = _calculate_k_min(x_train.shape[0])
    ...
    return _train_models(classifiers_hyper, x_train, y_train, x_valid, y_valid, classification_task)
...
def regression_for_different_regressors(x_train, y_train, x_valid, y_valid):
    ...
    return _train_models(regressors_hyper, x_train, y_train, x_valid, y_valid, regression_task)


def _calculate_max_tree_depth(n_features):
    """
...
def _calculate_k_min(n_samples):
    """
...
def _k_fold_cross_validation(model, x, y, hyperparam_name, value):
    """
...
def _train_w_best_hyperparam(model_hyper, x, y):
    """
    ...
    if hyperparam_name:
        accuracies = [_k_fold_cross_validation(model, x, y, hyperparam_name, value) for value in hyperparam_values]
...
def _train_model(model_hyper, x_train, y_train, x_valid, y_valid, prediction_task):
    """
    ...
    """
    model, hyperparam, best_hyperparam_value = _train_w_best_hyperparam(model_hyper, x_train, y_train)
...
def _train_models(models_hyper, x_train, y_train, x_valid, y_valid, prediction_task):
    """
    ...
    for regressor_hyper in models_hyper:
        training_results.append(
            _train_model(regressor_hyper, x_train, y_train, x_valid, y_valid, prediction_task))
