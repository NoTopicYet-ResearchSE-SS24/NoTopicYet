...
def test_load_pretrained_classifiers_seed_42(ml_data_func: Callable[..., MLData]) -> None:
...
def test_load_pretrained_regressors_seed_42(ml_data_func: Callable[..., MLData]) -> None:
    ...
    error = round(root_mean_squared_error(data.valid.y, logistic_regressor.predict(data.valid.x)), 3)
    ...
    error = round(root_mean_squared_error(data.valid.y, logistic_regressor_cv.predict(data.valid.x)), 3)
...
def test_train_model_for_regressionn_seed_42(
    ...
    best_model = load_model(path_to_best_model)
    error = round(root_mean_squared_error(data.valid.y, best_model.predict(data.valid.x)), 3)
